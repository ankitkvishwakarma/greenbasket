...
export default function ProductGrid() {
  ...
  const handleAdd = async (e, p) => {
    ...
    flyToCart(p.image, { x: rect.left, y: rect.top });
    ...
    <div className="px-8 md:px-20 py-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
        ...
        {products.map((p, idx) => (
          ...
          >
            <img src={p.image} className="w-28 h-28 mx-auto object-contain" />

            <h3 className="mt-3 font-semibold dark:text-white">{p.name}</h3>
            <p className="text-green-600 font-bold">₹{p.price} / {p.unit}</p>
